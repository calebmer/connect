...
export class PGGroupCollection implements GroupCollection {
  ...
  async get({groupID}: GroupMembership): Promise<Group> {
    const {
      ...
      sql`SELECT slug, name FROM "group" WHERE id = ${groupID}`,
    );
    if (row === undefined) {
      throw new Error(
  ...
  async getPosts(
    ...
    const {rows} = await PGPostPagination.query(this.client, {
      ...
      extraCondition: sql`group_id = ${groupID}`,
      range,
    });
    return rows.map(row => ({
      id: row.id,
  ...
  async getProfiles(
    ...
    const {rows} = await this.client.query(sql`
      ...
      WHERE
        group_member.account_id = ANY (${accountIDs}) AND
  ...
  async sharedMembership(
    ...
    const {rowCount} = await this.client.query(sql`
      ...
      WHERE
        group_member1.account_id = ${accountID1} AND
        group_member2.account_id = ${accountID2}
      LIMIT 1
    `);
    return rowCount > 0;
  }
